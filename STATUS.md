...
button. There is currently no voice input/output and no in-headset text entry
— see README.md's "Talking to Sophie in the browser" section for the current
state. The rest of this file (memory backend, WebXR scene, avatars) is
unaffected and still accurate as of the date above.
